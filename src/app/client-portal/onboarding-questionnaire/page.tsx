...
export default function OnboardingQuestionnairePage() {
  ...
  // Helper function to find unanswered questions
  const getUnansweredQuestions = () => {
    const unanswered: Array<{ question: OnboardingQuestion; section: number }> = [];
    
    ONBOARDING_QUESTIONS.forEach(question => {
      // Check if question should be shown (handle conditional logic)
      ...
      if (question.conditionalLogic) {
        ...
        if (question.conditionalLogic.condition === 'equals') {
          shouldCheck = dependsOnAnswer === question.conditionalLogic.value;
        } else if (question.conditionalLogic.condition === 'contains') {
          shouldCheck = Array.isArray(dependsOnAnswer) && dependsOnAnswer.includes(question.conditionalLogic.value);
        }
      }
      
      // Only check required questions that should be shown
      if (shouldCheck && question.required) {
        const answer = responses[question.id];
        if (answer === undefined || answer === null || answer === '') {
          unanswered.push({ question, section: question.section });
        }
      }
    });
  ...
  const handleSubmitToCoach = async () => {
    ...
    // Check if all sections are complete (9 sections now, measurements removed)
    if (progress.completedSections.length < 9) {
      const incompleteSections = 9 - progress.completedSections.length;
  ...
  const renderQuestion = (question: OnboardingQuestion) => {
    ...
    // Check if this required question is unanswered
    const isUnanswered = question.required && (answer === undefined || answer === null || answer === '');
    ...
    return (
      <div key={question.id} className={`mb-8 ${isUnanswered ? 'bg-yellow-50 border-2 border-yellow-400 rounded-xl p-6' : ''}`}>
        <label className="block text-xl font-bold text-gray-900 mb-4">
          ...
          {isUnanswered && (
            <span className="ml-2 text-xs font-semibold text-yellow-700 bg-yellow-200 px-2 py-1 rounded">
          ...
          <div className="max-w-4xl mx-auto">
            ...
            {(() => {
              ...
              if (unanswered.length > 0) {
                return (
                  <div className="mb-8 bg-yellow-50 border-2 border-yellow-400 rounded-2xl p-6 shadow-sm">
                    <div className="flex items-start">
                      ...
                      <div className="flex-1">
                        <h3 className="text-lg font-semibold text-yellow-900 mb-2">
                          {unanswered.length} Required Question{unanswered.length !== 1 ? 's' : ''} Still Unanswered
                        </h3>
                        <p className="text-sm text-yellow-800 mb-3">
                          Please complete the following required questions to finish your onboarding:
                        </p>
                        <div className="space-y-2">
                          {unanswered.map(({ question, section }) => {
                            const sectionName = ONBOARDING_SECTIONS.find(s => s.id === section)?.name || `Section ${section}`;
                            return (
                              <button
                                key={question.id}
                                onClick={() => handleSectionChange(section)}
                                className="block w-full text-left p-3 bg-white rounded-lg border border-yellow-300 hover:border-yellow-500 hover:bg-yellow-100 transition-colors"
                              >
                                <div className="flex items-center justify-between">
                                  <div className="flex-1">
                                    <span className="text-xs font-semibold text-yellow-700 uppercase">{sectionName}</span>
              ...
              <div className="mb-6 pb-4 border-b border-gray-200">
                ...
                <p className="text-gray-600 mt-1">
                  {(() => {
                    ...
                    const conditionalQuestions = sectionQuestions.filter(q => q.conditionalLogic);
                    if (conditionalQuestions.length > 0) {
                      return `${alwaysVisibleQuestions.length}-${sectionQuestions.length} questions (some may appear based on your answers)`;
                    }
                    return `${sectionQuestions.length} question${sectionQuestions.length !== 1 ? 's' : ''}`;
                  })()}
                </p>
              </div>
