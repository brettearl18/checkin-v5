...
import { getCheckInWindowClose24hReminderEmailTemplate } from '@/lib/email-templates';
import { calculateWindowCloseTime, DEFAULT_CHECK_IN_WINDOW } from '@/lib/checkin-window-utils';

export const dynamic = 'force-dynamic';

/**
 * POST /api/scheduled-emails/check-in-window-close-24h
 * 
 * Sends reminder emails 24 hours before check-in windows close
 * This should be called every hour to catch windows that close in 24 hours
 */
export async function POST(request: NextRequest) {
  try {
    ...
    for (const doc of assignmentsSnapshot.docs) {
      ...
      if (assignmentData.windowClose24hReminderSent) {
        results.skipped++;
        continue;
      }
      ...
      // Check if window closes in 24 hours (23-25 hour window to account for hourly runs)
      const in23Hours = new Date(now.getTime() + 23 * 60 * 60 * 1000);
      const in25Hours = new Date(now.getTime() + 25 * 60 * 60 * 1000);

      if (windowCloseTime < in23Hours || windowCloseTime >= in25Hours) {
        results.skipped++;
        continue;
      }
      ...
      // Send email
      try {
        const { subject, html } = getCheckInWindowClose24hReminderEmailTemplate(
          clientName,
        ...
        await sendEmail({
          ...
          html,
          emailType: 'check-in-window-close-24h-reminder',
          metadata: {
            assignmentId: doc.id,
        ...
        await db.collection('check_in_assignments').doc(doc.id).update({
          windowClose24hReminderSent: true,
          windowClose24hReminderSentAt: new Date(),
        });

        results.sent++;
      } catch (error) {
        console.error(`Error sending 24h window close reminder to ${clientEmail}:`, error);
    ...
    console.error('Error processing 24h window close reminders:', error);
    return NextResponse.json(
      {
        success: false,
        message: 'Failed to process 24h window close reminders',
