...
import { getCheckInWindowClose1hReminderEmailTemplate } from '@/lib/email-templates';
import { calculateWindowCloseTime, DEFAULT_CHECK_IN_WINDOW } from '@/lib/checkin-window-utils';

export const dynamic = 'force-dynamic';

/**
 * POST /api/scheduled-emails/check-in-window-close-1h
 * 
 * Sends reminder emails 1 hour before check-in windows close
 * This should be called every hour (or more frequently) to catch windows that close in 1 hour
 */
export async function POST(request: NextRequest) {
  try {
    ...
    for (const doc of assignmentsSnapshot.docs) {
      ...
      if (assignmentData.windowClose1hReminderSent) {
        results.skipped++;
        continue;
      }
      ...
      // Check if window closes in 1 hour (50-70 minute window to account for scheduling)
      const in50Minutes = new Date(now.getTime() + 50 * 60 * 1000);
      const in70Minutes = new Date(now.getTime() + 70 * 60 * 1000);

      if (windowCloseTime < in50Minutes || windowCloseTime >= in70Minutes) {
        results.skipped++;
        continue;
      }
      ...
      // Send email
      try {
        const { subject, html } = getCheckInWindowClose1hReminderEmailTemplate(
          clientName,
        ...
        await sendEmail({
          ...
          html,
          emailType: 'check-in-window-close-1h-reminder',
          metadata: {
            assignmentId: doc.id,
        ...
        await db.collection('check_in_assignments').doc(doc.id).update({
          windowClose1hReminderSent: true,
          windowClose1hReminderSentAt: new Date(),
        });

        results.sent++;
      } catch (error) {
        console.error(`Error sending 1h window close reminder to ${clientEmail}:`, error);
    ...
    console.error('Error processing 1h window close reminders:', error);
    return NextResponse.json(
      {
        success: false,
        message: 'Failed to process 1h window close reminders',
