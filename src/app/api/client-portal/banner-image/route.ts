...
/**
 * POST /api/client-portal/banner-image
 * Upload banner image for a client's dashboard
 */
  ...
  try {
    ...
    if (!user.isClient) {
      return NextResponse.json(
        { success: false, message: 'Only clients can upload banner images' },
        { status: 403 }
      );
    }
    ...
    const fileName = `banner-images/${clientId}/${Date.now()}_${file.name}`;
    ...
    // Update client document with banner image URL
    ...
    logInfo('Banner image uploaded successfully', { userId: user.uid, clientId });
